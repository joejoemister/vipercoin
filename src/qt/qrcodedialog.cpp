...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("VIPER")
    updateDisplayUnit();
}
...
{
    QString ret = QString("vipercoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VIPER unit input in lnReqAmount, we generate the URI with VIPER as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::VIPER, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
